...
//!
//! Ported from wolf's `zebra-chain/src/primitives/wcash_address.rs`
//! (w-cash/wolf) with the Zebra-specific conversion tests trimmed. Keep this
//! file in sync with the upstream module; the codec is consensus-adjacent.
//!
...
/// Errors returned while converting wallet addresses at the Wcash boundary.
///
/// Mirrors wolf's `wcash-wallet/src/address.rs` boundary error split.
...
pub enum WalletAddressError {
    ...
    WrongNetwork,
    /// The receiver payload could not be converted to a wallet address type.
    #[error("invalid Wcash address receiver payload: {0}")]
    Conversion(String),
}

/// Encodes a wallet-derived Unified Address in the Wcash namespace.
///
/// The receiver payloads are unchanged; only the textual namespace (HRP and
/// F4Jumble padding domain) differs from the Zcash encoding.
pub fn encode_wallet_unified_address(
    ...
    let container = address
        ...
        .map_err(|error| WalletAddressError::Conversion(error.to_string()))?;
    Ok(container.encode())
}

/// Encodes a wallet-derived transparent receiver in the Wcash namespace.
pub fn encode_wallet_transparent_address(
...
/// Decodes a Wcash recipient into the address type used by the wallet stack.
///
/// Zcash textual addresses are rejected (`WcashAddressParseError::NotWcash`),
/// and the address must belong to `network`.
pub fn decode_wallet_address(
    encoded: &str,
    network: NetworkType,
) -> Result<zcash_keys::address::Address, WalletAddressError> {
...
mod tests {
    ...
    fn local_miner_unified_address_has_wcash_encoding() {
        ...
        let converted = address
            .clone()
            .convert_if_network::<zcash_keys::address::Address>(NetworkType::Regtest)
            .unwrap();
        let zcash_keys::address::Address::Unified(unified) = converted else {
            panic!("a Wcash UA must convert to a unified receiver");
        };
    ...
    fn rejects_wrong_network_during_conversion() {
        ...
        let result = address.convert_if_network::<WcashAddress>(NetworkType::Test);
        assert!(matches!(
            result,
    ...
    #[test]
    fn converts_into_zcash_keys_address_type() {
    ...
    #[test]
    fn boundary_round_trips_wcash_unified_recipient() {
        let expected = WcashAddress::from_unified(NetworkType::Regtest, orchard_unified_address())
            .expect("the Orchard-only fixture is supported")
            .encode();

        let decoded = decode_wallet_address(&expected, NetworkType::Regtest).unwrap();
        let zcash_keys::address::Address::Unified(unified) = decoded else {
            panic!("a Wcash UA must decode to a unified receiver");
        };
        assert_eq!(
            encode_wallet_unified_address(&unified, NetworkType::Regtest).unwrap(),
            expected
        );
    }

    #[test]
    fn boundary_rejects_zcash_and_cross_network_recipients() {
        // The Zcash namespace must not decode as a Wcash recipient.
        let zcash_encoded = "uregtest1pszqlgxaf5w8mu2yd9uygg8cswp0ec4f7eejqnqc35tztw4tk0sxnt3pym2f3s2872cy2ruuc5n8y9cen5q6ngzlmzu8ztrjesv8zm9j";
        assert!(matches!(
            decode_wallet_address(zcash_encoded, NetworkType::Regtest),
            Err(WalletAddressError::Parse(WcashAddressParseError::NotWcash))
        ));

        // A Wcash address from another Wcash network is rejected.
        let testnet_transparent =
            WcashAddress::from_transparent_p2pkh(NetworkType::Test, [7; 20]).encode();
        assert!(matches!(
            decode_wallet_address(&testnet_transparent, NetworkType::Regtest),
            Err(WalletAddressError::WrongNetwork)
        ));
    }

    #[test]
    fn boundary_encodes_transparent_receivers_in_the_wcash_namespace() {
        let p2pkh = transparent::address::TransparentAddress::PublicKeyHash([7; 20]);
        let encoded = encode_wallet_transparent_address(&p2pkh, NetworkType::Regtest);
        assert_eq!(
            encoded.parse::<WcashAddress>().unwrap(),
            WcashAddress::from_transparent_p2pkh(NetworkType::Regtest, [7; 20])
        );
    }
